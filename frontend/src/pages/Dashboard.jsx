...
import { FaUsers, FaCheckCircle, FaChartLine, FaTrash, FaSearch, FaBars } from "react-icons/fa";
...
function Dashboard() {
  ...
  const addLead = async () => {
    ...
    try {
      await axios.post("https://leadflow-crm-yvk1.onrender.com/api/leads", {
        companyName,
        contactPerson,
        status,
      });
  ...
  const deleteLead = async (id) => {
    try {
      await axios.delete(`https://leadflow-crm-yvk1.onrender.com/api/leads/${id}`);
      fetchLeads();
    } catch (error) {
      console.log(error);
    }
    ...
    <div className="flex h-screen bg-zinc-50 overflow-hidden">
      ...
      <div className="flex-1 p-4 md:p-8 mt-[80px] md:mt-0 overflow-y-auto overflow-x-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl border border-zinc-200/60 shadow-sm p-6 hover:-translate-y-1 hover:shadow-md transition-all duration-300">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-zinc-500 text-sm font-medium">Total Leads</p>
                <h2 className="text-3xl font-bold mt-3 text-zinc-900">{leads.length}</h2>
              </div>
          ...
          <div className="bg-white rounded-xl border border-zinc-200/60 shadow-sm p-6 hover:-translate-y-1 hover:shadow-md transition-all duration-300">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-zinc-500 text-sm font-medium">Interested</p>
          ...
          <div className="bg-white rounded-xl border border-zinc-200/60 shadow-sm p-6 hover:-translate-y-1 hover:shadow-md transition-all duration-300">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-zinc-500 text-sm font-medium">Closed Deals</p>
                <h2 className="text-3xl font-bold mt-3 text-zinc-900">
        ...
        <div className="bg-white mt-8 rounded-xl border border-zinc-200/60 shadow-sm p-6 md:p-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
            <div>
              ...
              <p className="text-zinc-500 text-sm mt-1">Create and manage new client leads</p>
            </div>
          </div>
        ...
        <div className="bg-white mt-8 rounded-xl border border-zinc-200/60 shadow-sm p-6 md:p-8 overflow-hidden">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
            <div>
              <h2 className="text-xl font-bold text-zinc-900 tracking-tight">Recent Leads</h2>
              <p className="text-zinc-500 mt-1 text-sm">Manage all your latest leads</p>
            </div>
          </div>
            ...
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-zinc-100 text-zinc-500 text-xs uppercase tracking-wider">
                  <th className="text-left py-4 font-semibold">Company</th>
                  <th className="text-left py-4 font-semibold">Contact Person</th>
              ...
              <tbody>
                {filteredLeads.map((lead) => (
                  ...
                  >
                    <td className="py-4 text-sm font-medium text-zinc-900">{lead.companyName}</td>
                    <td className="py-4 text-sm text-zinc-500">{lead.contactPerson}</td>
                    <td className="py-4">
